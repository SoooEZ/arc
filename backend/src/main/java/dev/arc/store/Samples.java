...
@Component
public class Samples implements ApplicationRunner {
    private final RuleStore store;
    private final RuleService service;
    public Samples(RuleStore store, RuleService service) { this.store = store; this.service = service; }
    private static Node node(String id, String type, String label, double x, double y, String expression, String output) {
        return new Node(id, type, label, new Position(x, y), expression, output, null, null, null);
    }
    private static Edge edge(String source, String target, String branch) { return new Edge(source + "-" + branch + "-" + target, source, target, branch); }
    public static Definition blank(String kind) {
        if (kind.equals("RULE")) return new Definition(1, List.of(new Input("amount", "NUMBER", true, 100)), List.of(
            ...
            List.of(edge("input", "condition", "next"), edge("condition", "yes", "true"), edge("condition", "no", "false")));
        return new Definition(1, List.of(new Input("amount", "NUMBER", true, 100)), List.of(
    ...
    public void run(ApplicationArguments args) {
        ...
        Definition pricing = new Definition(1, List.of(new Input("orderTotal", "NUMBER", true, null), new Input("customerTier", "STRING", true, null)), List.of(
            ...
            new Node("premium", "REFERENCE", "Premium discount", new Position(100, 320), null, "price", "apply-discount", 1, Map.of("amount", "orderTotal", "rate", "0.2")),
            node("threshold", "CONDITION", "Order over $100?", 520, 320, "orderTotal >= 100", null),
            new Node("standard", "REFERENCE", "Volume discount", new Position(380, 500), null, "price", "apply-discount", 1, Map.of("amount", "orderTotal", "rate", "0.1")),
