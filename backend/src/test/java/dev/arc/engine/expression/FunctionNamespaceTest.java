...
class FunctionNamespaceTest {
  ...
  void lazyAndExcelCallsKeepTheirExistingSemanticsAndDollarStringsStayLiteral() {
    ...
    assertThat(eval("$IFERROR($ERROR.TYPE(1), 7)")).isEqualTo(eval("IFERROR(ERROR.TYPE(1), 7)"));
  ...
  @Test
  void collectionFunctionsRetainLexicalLocalScopesAndLegacyCalls() {
    var expression =
        Expressions.compile(
    ...
    assertThat(
            Expressions.compile("$MAP(items, item, SUM(MAP(item.parts, item, item.price))) + item")
                .variables())
