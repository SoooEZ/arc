...
class ExpressionsTest {
    private Object eval(String expression) { return Expressions.evaluate(expression, Map.of()); }
    @Test void arithmeticIsDecimalAndRespectsPrecedence() {
    ...
    @Test void roundingAndFunctionsAreDeterministic() {
    ...
    @Test void logicalOperatorsShortCircuit() {
    ...
    @Test void stringsAndInputs() {
        assertThat(Expressions.evaluate("customerTier == \"premium\"", Map.of("customerTier", "premium"))).isEqualTo(true);
        assertThat(eval("'hello' != 'world'")).isEqualTo(true);
        assertThat(eval("\"hello\\nworld\"")).isEqualTo("hello\nworld");
        assertThat(Expressions.compile("amount * (1 - rate)").variables()).containsExactlyInAnyOrder("amount", "rate");
    }
    @Test void invalidExpressionsFailClearly() {
        ...
        assertThatThrownBy(() -> eval("round(1, 99)")).hasMessageContaining("0 to 12");
        assertThatThrownBy(() -> eval("round(1, 0.5)")).hasMessageContaining("integer");
        assertThatThrownBy(() -> eval("max(1)")).hasMessageContaining("argument count");
        assertThatThrownBy(() -> eval("true + 1")).hasMessageContaining("Expected a number");
    }
    @Test void arbitraryCodeAndExcessiveWorkAreRejected() {
        for (String expression : new String[]{"Runtime.getRuntime()", "new ProcessBuilder()", "T(java.lang.Runtime)", "system('whoami')", "1e9999", "1 + ".repeat(200) + "1", "(".repeat(60) + "1" + ")".repeat(60)}) {
