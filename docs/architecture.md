...
Expressions read upstream Formula/Reference outputs as ordinary variables. A direct published Formula call uses `@rule-id:version(arguments)`, for example `@apply-discount:1(amount, rate)`. The expression itself stores the immutable ID/version pin, so no graph metadata or display-name lookup is needed. Only `FORMULA` library entries qualify. Positional arguments follow the pinned input order; trailing inputs may be omitted if their required/default/source contracts permit it. Explicit null is distinct from omission.

The expression compiler records Formula calls separately from variable dependencies. Static validation checks each call's kind, pin and argument count without executing it or fetching source values. Runtime dispatch enters the same graph session as Reference nodes, so nested Formula calls share deadline, recursion/step/source-read limits, error locations and trace. Lazy expression branches retain their ordinary behavior: an inactive call performs no child execution or source read. Source-contract validation traverses both Reference nodes and Formula-call dependencies. Calls remain ordinary expression text in graph JSON, ARC Script and node fragments.
