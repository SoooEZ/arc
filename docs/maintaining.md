...
The Rule library owns browsing and pagination; the collapsible navigation does not duplicate its catalog. Card previews load only for the mounted page and cancel on page/search changes. A preview represents every actual node and valid connection, including fan-out and joins. Small graphs show labels; complex graphs show an overview with a direct route to the full canvas. Preview layout is local presentation data and never changes saved positions.
...
Public function names and signatures include `$`; internal dispatch names remain unprefixed. Catalog snippets escape each literal dollar as `\$` for Monaco while preserving `${n:...}` argument placeholders. Include nested calls in that rule. Completion ranges consume an already typed `$` exactly once, and function hover must not mistake a same-named input for a call. The parser rejects unprefixed calls in every entry point, including old stored expressions. This is an intentional language compatibility change; fixing an old rule requires editing its draft and publishing a new version, never rewriting published JSON in place. Parameter declarations retain `Identifiers` syntax, excluding whitespace and `$` in rule and source inputs.

`domain/expressionSymbols.ts` classifies visible names for model-scoped Monaco semantic colors. It is presentation logic, not a second expression evaluator. Preserve parameter/result metadata from `VariableOption`, skip literals/comments, classify only a dotted path's root, and respect collection-local shadowing. Inline editors use the authoritative available scope; script editors color declarations without claiming execution validity. Providers return no tokens for unrelated models and are disposed with the editor. Published Formula metadata is loaded on explicit `@` completion, hover or picker use, with bounded pages and an editor-local cache. Async insertion rechecks the model, its version, selection and read-only state before writing. Inspector sections share default-expanded MUI accordions and keep the variable preview/pinned overlay outside the editable layout; opening the variable action must not toggle the accordion.
...
Reference node rule/version pickers combine server search and scroll-driven paging in one autocomplete. Each search owns its page sequence; failed appends retain loaded options, and a retry uses the same offset. Selection stays independent of search results, so a filtered or off-page pinned version remains unchanged until the user selects another option. Rule version search filters numeric version text on the server before counting and paging. Preserve the first character typed into a closed dropdown and the user's current scroll position when options append; the [picker workflow tests](../frontend/tests/reference-picker.spec.ts) exercise both event-ordering cases. The drawer and staged node dialog share `NodeIdentity`; the dialog header edits only its local draft until Apply and keeps a stable accessible dialog name while renaming.
...
`ExpressionField` lazy-loads an inline Monaco editor and the larger `ExpressionDialog`. Both reuse Code studio's model-scoped completion providers, offering only in-scope variable names and no graph-module snippets. Inline editors request the function catalog on first focus, retain incomplete expression text in the caller's draft, and use Tab for completion, snippet arguments and indentation. Disable font ligatures so operators such as `==` display every entered character. `/studio/expression/check` parses syntax and reports dependencies, while the application layer validates direct Formula pins and arity without evaluating values or fetching source data; the dialog cancels obsolete checks and preserves its caller's draft until Apply. Runtime type checks still belong to preview/execution. Preview and the published API playground share a lazy JSON editor; their controllers retain the raw buffer, including invalid JSON, and continue to own parsing and execution request identity.
