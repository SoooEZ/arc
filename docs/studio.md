...
Clicking a node opens its inspector. The header keeps the node icon, inline **Node name** field and `</>` action in one row, with the node type below its icon. Editing the name updates the canvas label immediately. The header also offers a **Delete node** icon with a hover label; Input cannot be deleted. Inspector sections use bold headings and start expanded; click a heading to collapse its section. **Expression**, **Output As**, and other mapping sections show an **Available variables** icon at the right of their heading. Hover for a preview or click to keep the list open; Escape, the close button, or a click outside dismisses it. Formula, Reference and Transform result names live under **Output As**. Variable choices follow connected upstream paths, including the path from Input. Connect a new Switch to Input or an upstream calculation before choosing its variables; disconnected nodes have none. **Functions & editor** shows the same available names for expressions. Variable dropdowns display `name (type) - node name`; computed outputs with no declared type use `result`.

Reference nodes use **Select Rule** for the published rule and version. Its heading contains an open icon for viewing the selected pin and an info icon explaining version pinning. **Parameters for Rule** shows one card per declared input, with its required/optional status and type; mapping controls still accept an upstream variable, constant, expression, or the callee’s default/source.
